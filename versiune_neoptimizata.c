...
void setup() {
  ...
  generate_image();  // generăm imaginea în loc să o copiem din fingerprint_image.h
  ...
  Serial.print("SSIM: "); Serial.println(calculate_ssim(), 4);
    ...
    // UNOPTIMIZED - versiune mai lentă intenționat
  memset(histogram, 0, sizeof(histogram));
  startTime = micros();

  // Histogramă neoptimizată și intenționat ineficientă
  for (int i = 0; i < IMG_SIZE * IMG_SIZE; ++i) {
    for (int h = 0; h < HIST_BINS; ++h) {
      if (h == image_copy[i]) {
        histogram[h]++;
        break; // simulează o căutare ineficientă
      }
    }
  }
